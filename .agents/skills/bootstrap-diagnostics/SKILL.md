...
- `HOME_SUMMARY: this home has never published state/cockpit-observation.json` or `... has not been republished since <stamp>` - this home's structured summary and redacted observation publication has failed repeatedly, and the line carries the failure count and the newest recorded reason from `state/.home-summary-refresh.log`.
