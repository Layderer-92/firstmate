...
The same refresh derives `state/cockpit-observation.json` from that already validated summary as a path-, text-, identity-, and lineage-free `internal_non_sensitive` projection; `bin/fm-cockpit-observation.sh` owns its exact machine contract and read-only stdout validation.
